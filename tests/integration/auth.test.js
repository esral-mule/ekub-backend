...
describe('AUTHENTICATION APIS', async () => {
  ...
  beforeEach(async () => {
    dbUser = {
      phoneNumber: 'branstark@gmail.com',
      password: 'mypassword',
      name: 'Bran Stark',
      role: 'admin',
    };
  ...
  describe('POST /v1/auth/register', () => {
    ...
    it('Should report error when phoneNumber already exists', async () => {
      ...
      expect(res.body.message).to.equal('Phonenumber is already in use by another account');
    ...
    it('Should report validation error when fileds are invalid', async () => {
      ...
      expect(data2.field).to.be.equal('phoneNumber');
      expect(data2.location).to.be.equal('body');
      expect(data2.messages).to.be.equal('phoneNumber must be a valid phoneNumber');
    ...
    it('Should report validation error when fields are not passed', async () => {
      ...
      expect(data2.field).to.be.equal('phoneNumber');
      expect(data2.location).to.be.equal('body');
      expect(data2.messages).to.be.equal('phoneNumber is required');
  ...
  describe('POST /v1/auth/login', () => {
    ...
    it('Should report invalid credential error', async () => {
      const invalidData = {
        phoneNumber: 'branstark12@gmail.com',
