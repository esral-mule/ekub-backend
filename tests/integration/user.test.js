...
describe('USERS APIS', async () => {
  ...
  beforeEach(async () => {
    dbUsers = {
      shekhar: {
        name: 'shekhar',
        phoneNumber: 'shekhar12@gmail.com',
        password: passwordHashed,
        role: 'admin',
      },
      michal: {
        name: 'Michal',
        phoneNumber: 'michal12@gmail.com',
        password: passwordHashed,
      },
      stephan: {
        name: 'stephan',
        phoneNumber: 'stephan12@gmail.com',
        password: passwordHashed,
      },
    };

    user = {
      phoneNumber: 'sousa.dfs@gmail.com',
      password,
      name: 'Daniel Sousa',
    };
  ...
  describe('POST /api/v1/users', () => {
    ...
    it('should report error when phoneNumber already exists ', async () => {
      user.phoneNumber = dbUsers.shekhar.phoneNumber;
      const res = await request(app)
        .post('/api/v1/users')
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .send(user)
      ...
      expect(field).to.be.equal('phoneNumber');
      expect(location).to.be.equal('body');
      expect(messages).to.be.equal('Phonenumber is already in use');
    });

    it('should report error when phoneNumber is not provided ', async () => {
      ...
      expect(field).to.be.equal('phoneNumber');
      expect(location).to.be.equal('body');
      expect(messages).to.be.equal('phoneNumber is required');
    });
  ...
  describe('GET /api/v1/users/:userID', () => {
    it('should get user', async () => {
      ...
      const id = (await User.findOne({ phoneNumber: 'shekhar12@gmail.com' }))._id;
      const res = await request(app)
        .get(`/api/v1/users/${id}`)
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .expect(200);
    ...
    it('should report error when logged user is not the same as the requested one', async () => {
      const id = (await User.findOne({ phoneNumber: dbUsers.michal.phoneNumber }))._id;
      const res = await request(app)
        .get(`/api/v1/users/${id}`)
        .set('Authorization', `Bearer ${userAccessToken}`)
        .expect(403);
  ...
  describe('PUT /v1/users/:userID', async () => {
    it('should replace user', async () => {
      delete dbUsers.shekhar.password;
      const id = (await User.findOne({ phoneNumber: dbUsers.shekhar.phoneNumber }))._id;
      const res = await request(app)
        .put(`/api/v1/users/${id}`)
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .send(user)
    ...
    it('should report error when phoneNumber not provided', async () => {
      delete user.phoneNumber;
    ...
    it('should report an error when Phonenumber trying to replace already exists', async () => {
      delete dbUsers.shekhar.password;
      user.phoneNumber = dbUsers.michal.phoneNumber;
      const id = (await User.findOne({ phoneNumber: dbUsers.shekhar.phoneNumber }))._id;
      const res = await request(app)
        .put(`/api/v1/users/${id}`)
        .set('Authorization', `Bearer ${adminAccessToken}`)
        ...
        .expect(400);
      expect(res.body.code).to.be.equal(400);
      expect(res.body.message).to.be.equal('Phonenumber is already in use by another account');
      const { field, location, messages } = res.body.errors[0];
      expect(field).to.be.equal('phoneNumber');
      expect(location).to.be.equal('body');
      expect(messages).to.be.equal('Phonenumber is already in use');
    ...
    it('should not replace role when Logged user is not admin', async () => {
      ...
      const id = (await User.findOne({ phoneNumber: dbUsers.stephan.phoneNumber }))._id;
      const role = 'admin';
      user.role = role;
      const res = await request(app)
        .put(`/api/v1/users/${id}`)
  ...
  describe('PATCH /v1/users/:userID', () => {
    ...
    it('should report error when phoneNumber trying to update already exists', async () => {
      user.phoneNumber = dbUsers.michal.phoneNumber;
      const id = (await User.findOne({ phoneNumber: dbUsers.shekhar.phoneNumber }))._id;
      const res = await request(app)
        .patch(`/api/v1/users/${id}`)
      ...
      expect(field).to.equal('phoneNumber');
      expect(location).to.equal('body');
      expect(messages).to.equal('Phonenumber is already in use');
    });
  });
});
